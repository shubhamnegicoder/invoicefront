...
class ListBill extends React.Component {
  ...
  onmodal = (data) => {
    ...
    data.forEach((item, index) => {
      ...
      dataArray.push(item.billNumber)
      dataArray.push(item.companyName)
      dataArray.push(item.customerName)
      dataArray.push(item.billDate.split("T")[0])
  ...
  handlePdf = (id, billNumber) => {
    // window.location.href = "./viewInvoice?type=listinvoice" + "&invoiceNo=" + billNumber;
  ...
  handleView = (id, billNumber) => {
    window.location.href = "./viewBill?id=" + id + "&billNumber=" +billNumber;
  }
  handleClose(id) {
    swal({
      text: "Are you sure that you want to cancel this Bill?",
      type: "warning",
      ...
      if (result.value) {
        axios
          .post("http://localhost:8080/editBill", { id: id, status: "Cancelled" })
          .then((res) => {
            this.List();
          })
      }
  ...
  List = () => {
    fetch("http://localhost:8080/allBillList?id=" + this.state.id, {
      method: "GET",
      cache: 'no-cache',
      mode: 'cors',
      headers: new Headers({
      ...
      .then(
        (result) => {
          console.log("listabc =kkkkkkkkkkkkkkkkkkkk ", result.data)
          var mainArray = [];
       ...
       <Button style={{ float: "right", backgroundColor: "#76323f", color: "white" }} aria-label="add" variant="fab" onClick={this.handleOpen} ><Search />
              ...
              content={
                <Table
                  ...
                  tableHead={["Bill Number", "Company", "Customer", "Date", "Amount", "Status", "Action"]}
